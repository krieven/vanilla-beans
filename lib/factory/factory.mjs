...
/**
 * @type {{[key:string]:AnyModule}}
 */
var modules = {}
/**
 * @type {{[key:string]:BeanSearchResult}}
 */
var builtins = {}
...
factory.cleanAttributes = true

/**
 * 
 * @param {AnyModule} module 
 * @returns 
 */
...
/**
 * @param {string | number} moduleKey
 */
var has = factory.has = function (moduleKey) {
    return !!modules[moduleKey]
}

/**
 * @param {string|null|undefined} ns
 * @param {string | undefined} tag
 * @param {{[x: string]: any;} | undefined} attributes
 * @param {[BeanDescriptor | string] | [] | undefined} children
 * @param {{} | undefined} appContext
 * @param {string} moduleSrc
 * @param {{[k:string]:Node} | {} | null | void | undefined} rootRef
 * @param {Element & { _BODY:Element } | null | void | undefined} root
 * 
 * @returns {Node}
 */
function create(ns, tag, attributes, children, appContext, moduleSrc, rootRef, root) {
    ...
    var def = searchBean(moduleSrc, strtoupper(tag))

    //создаём builtin - он сам знает, что делать со всеми входящими
    if (def && def.create) {
        return def.create(ns, tag, attributes, children, appContext, moduleSrc, rootRef)
    }
    //создаём компонент из фабики, используем только потомков из дескриптора компонента
    if (def && def.bean) {
        ...
        // @ts-ignore
        initNode(result, def.bean, appContext, factory(def.src), beanRef, def.src)
    }
    //сюда попадаем когда создаём конкретный узел DOM
    else {
        ...
        setAttributes(result, attributes, factory.cleanAttributes)
        mix(result, handlers, true, false)
    }

    rootRef = rootRef || {}

    // @ts-ignore
    createChildren(ns, result, children, appContext, moduleSrc, rootRef, root)

    if(!root && result._BODY && result._BODY != result && result._BODY._BODY && result._BODY._BODY != result._BODY) {
        result._BODY = result._BODY._BODY
    }
 ...
 * @param {Node | null | undefined | void} root
 */
function createChildren(ns, result, children, appContext, moduleSrc, rootRef, root) {
    var document = factory.document
    root = root || result
    ...
    children.forEach(function (item) {
        ...
        if (!item.t) {
            ...
            (currentContentBody).appendChild(document.createTextNode(item))
            return
        }

        // @ts-ignore потому что здесь и далее может быть только BeanDescriptor
        var child = create(ns, item.t, item.a, item.c, appContext, moduleSrc, rootRef, root)
        ...
        // @ts-ignore
        root._BODY = (typeof getA(item, NS.BDY) !== 'undefined') && child || root._BODY
 ...
 * @returns {BeanSearchResult | undefined}
 */
function searchBean(rootSrc, fullTagname) {
    if (builtins[fullTagname]) return builtins[fullTagname]
 ...
 * 
 * @param {AnyProps | void} trg
 * @param {AnyProps | void} mixin
...
//     onBeanStop: function () { },
//     onBeanUpdate: function (data, options, additional) { },
//     transformBeanData: function (data, options, additional) { }.
 ...
 * 
 * @param {*} node 
 * @param {(n:BeanInstance | Node) => undefined} foreach 
 * @returns 
 */
...
builtins[strtoupper(NS.NS) + 'ITERATOR'] = {
    create: function (ns, tag, attributes, templates, appContext, moduleSrc, rootRef) {
        ...
        setProperty(result, 'beanUpdate', function (
            /** @type {string | any[]} */ data,
            /** @type {any} */ options,
            /** @type {any} */ additional) {

            // @ts-ignore
            data = this.transformBeanData ? this.transformBeanData(data, options, additional) : data
            data = Array.isArray(data) ? data : []

            // @ts-ignore
            var children = this.children

            for (var r = 0; r < data.length || r < children.length;) {
 ...
 * не имеет смысла как внутренний элемент, только для расширения
 * 
...
builtins[strtoupper(NS.NS) + 'FRAGMENT'] = {
    create: function (ns, tag, attributes, children, appContext, moduleSrc, rootRef) {
        ...
        setProperty(result, 'beanUpdate', handlers.beanUpdate)
        return result;
    }
}

builtins[strtoupper(NS.NS) + 'TEXT'] = {
    create: function (ns, tag, attributes, children, appContext, moduleSrc, rootRef) {
        var document = factory.document
        var result = document.createTextNode(probe(function () { return children.join('') }).or(''))
        setProperty(result, 'beanUpdate', function (
            /** @type {any} */ data,
            /** @type {any} */ options,
            /** @type {any} */ additional) {

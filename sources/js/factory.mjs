...
/**
 * @constant {{[src]:{beans:{}, imports:{[key]:{src:string, type:string}}, src:string} || {script:string, src:string}}} modules
 */
var modules = {}
 ...
 * @returns {{with:(appContext: any) => {
 *      create:(tag: string, attributes: undefined | {[x:string]: any}, children: undefined | []) => (void | Node)
 * }}}
 */
export default function factory(moduleSrc) {
    moduleSrc = moduleSrc + ''
    if (!has(moduleSrc)) throw new Error('Module not found')
    return {
...
function create(tag, attributes, children, appContext, moduleSrc, rootRef) {
    ...
    if (def && def.bean) {
        var usetagAttributes = {}
        usetagAttributes[NS.USETAG] = attributes[NS.USETAG]

        var beanattr = mix(
            mix({}, def.bean.a), usetagAttributes
        )
        var beanRef = {}
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        if (!children[i].t) {
            (result.beanContentBody || result).appendChild(document.createTextNode(children[i]))
            continue
        }

        var child = create(children[i].t, children[i].a, children[i].c, appContext, moduleSrc, rootRef)
        var ref = getA(children[i], NS.REF)
        ref && setProperty(rootRef, ref, child, true)

        beanContentBody = (typeof getA(children[i], NS.BODY)!=='undefined') && child || beanContentBody
...
/**
 * @param {{ setAttribute: (arg0: string, arg1: any) => void; }} instance
 * @param {{ [x: string]: any; }} attr
 * @param {boolean} [clean]
 */
...
function factoryRequire(moduleSrc) {
    return function (moduleKey) {
        var evaluated
        try {
            evaluated = modules[modules[moduleSrc].imports[moduleKey].src].evaluated
        } catch (error) {
            throw new Error('JS module "' + moduleKey + '" does not imported in "' + moduleSrc + '"')
        }
        return evaluated
    }
}

...
- **src** - относительный или абсолютный адрес подключаемого ресурса (URL или имя файла, относительно текущего модуля или document.baseURI)
...
Использование атрибута **xmlns** означает, что элемент и его поддерево будут создаваться с соответсвующим **NAMESPACE**. Для отключения **xmlns** в поддереве такого элемента можно использовать **xmlns="html"**.
...
Для того, чтобы Bean мог реагировать на эти события предусмотрены жендлеры жизненного цикла

...
- **beans-body** - указывает элемент в поддереве бина, в который будут монтироваться ноды извне, в бине может располагаться только один элемент с тэгом **beans-body**
    Служит для превращения бина в декоратор. Логика поиска места внутри бина такая:
    - по умолчанию ноды извне вставляются в корневой узел бина
    - если узел помечен как **beans-shadow**, то корнем для вставки в него является его **shadowRoot**
    - если на каком-либо элементе в описании бина установлен атрибут **beans-body** то ноды извне должны вставляться в него, или если у него есть shadowRoot, то вего shadowRoot
